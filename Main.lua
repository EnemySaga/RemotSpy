...
local Configuration = {
	...
	RepoUrl = [[https://raw.githubusercontent.com/EnemySaga/RemotSpy/refs/heads/main]],
	ParserUrl = [[https://raw.githubusercontent.com/IEnemyFiles/Roblox-Parser/refs/heads/main/Main.luau]],
    Directory = "Sigma Spy"
}

--// Initialization safety timeout (60 seconds)
local InitTimeout = tick() + 60
local function CheckTimeout()
	if tick() > InitTimeout then
		warn("[!] Script initialization timeout - script took too long to load")
		return true
	end
	return false
end
...
local FontJsonFile
if FontContent then
	FontJsonFile = Files:CreateFont("ProggyClean", FontContent)
end
