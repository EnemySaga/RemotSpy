...
function Generation:LoadParser(ModuleUrl: string)
	local Success, Result = pcall(function()
		local HttpContent = game:HttpGet(ModuleUrl)
		return loadstring(HttpContent, "Parser")()
	end)
	
	if not Success then
		warn("[!] Failed to load parser module!")
		warn(Result)
		--// Create stub parser to prevent crashes
		ParserModule = {
			Modules = {
				Formatter = {
					MakePrintable = function(self, str) return tostring(str) end,
