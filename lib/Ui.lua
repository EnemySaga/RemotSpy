...
function Ui:AskUser(Config: AskConfig): string
	...
	local MaxTimeout = 600 --// 10 seconds with default wait(0.1) or 60 seconds with wait()
	...
	repeat 
		...
		if TimeoutCounter > MaxTimeout then
			Answered = Config.Options[1] or "Yes"
			pcall(function() ModalWindow:ClosePopup() end)
			break
		end
	until Answered
	return Answered
