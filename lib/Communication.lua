...
function Module:CreateCommChannel(): (number, BindableEvent)
    ...
    Channel.Name = tostring(ChannelId)  --// Store as string for consistency
...
function Module:GetCommChannel(ChannelId: number): BindableEvent?
    ...
    local Parent = self:GetHiddenParent()
    local ChannelName = tostring(ChannelId)  --// Convert to string for FindFirstChild
    local Channel = Parent:FindFirstChild(ChannelName)

    --// Validate channel exists before wrapping
    if not Channel then
        warn("[!] Communication channel not found:", ChannelName)
        return nil, false
    end
