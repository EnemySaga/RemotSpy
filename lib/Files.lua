...
function Files:UrlFetch(Url: string): string
    ...
    if StatusCode >= 400 then
        warn("[!] HTTP request failed with status code:", StatusCode)
        warn(" >", Url)
        return ""
    end
...
function Files:LoadCustomasset(Path: string): string?
	...
	if not AssetSuccess then return end
...
function Files:GetModule(Name: string, TemplateName: string): string
	...
	if TemplateName then
		...
		--// Check if it successfuly loads with error handling
		local Success, Content = pcall(readfile, Path)
		if Success and Content then
			local LoadSuccess = loadstring(Content)
			if LoadSuccess then return Content end
		end
